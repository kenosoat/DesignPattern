...
public class DelItemVisitor implements ItemVisitor<ProductItem>{
    ...
    public ProductItem visitor(ProductItem productItem) {
        ...
        if(currentItem.getId() == productItem.getPid()){
            currentItem.delItem(productItem);
            return currentItem;
        }
        delItem(currentItem,productItem);
        return currentItem;
    }

    private void delItem(ProductItem currentItem, ProductItem productItem) {
        for(ProductItem item:currentItem.getChild()){
            if(item.getId() == productItem.getPid()){
                item.delItem(productItem);
                break;
            }else{
                delItem(item,productItem);
            }
        }
    }
}
